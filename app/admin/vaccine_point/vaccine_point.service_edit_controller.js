// blank controller
vaccine_point.controller('VaccinePointServiceEditController', VaccinePointServiceEditController);

function VaccinePointServiceEditController($scope, $http) {


    var vaccine_point_service_id = document.getElementsByName('vaccine_point_service_id')[0].value;
    console.log(vaccine_point_service_id);

    $http
        .get(window.location.origin+"/service/api/vaccine-point/get-selected-service/" + vaccine_point_service_id, {
        ...
        $http
        .get(window.location.origin+"/service/api/vaccine-point/get-selected-subservice/" + vaccine_point_service_id, {
            transformRequest: angular.identity,
            headers: {'Content-Type': undefined, 'Process-Data': false}
        })
        .then(function(response){
