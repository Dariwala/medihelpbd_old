// blank controller
gym.controller('GymServiceEditController', GymServiceEditController);

function GymServiceEditController($scope, $http) {


    var gym_service_id = document.getElementsByName('gym_service_id')[0].value;
    console.log(gym_service_id);

    $http
        .get(window.location.origin+"/service/api/gym/get-selected-service/" + gym_service_id, {
        ...
        $http
        .get(window.location.origin+"/service/api/gym/get-selected-subservice/" + gym_service_id, {
            transformRequest: angular.identity,
            headers: {'Content-Type': undefined, 'Process-Data': false}
        })
        .then(function(response){
