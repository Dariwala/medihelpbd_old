// blank controller
parlour.controller('ParlourServiceEditController', ParlourServiceEditController);

function ParlourServiceEditController($scope, $http) {


    var parlour_service_id = document.getElementsByName('parlour_service_id')[0].value;
    console.log(parlour_service_id);

    $http
        .get(window.location.origin+"/service/api/parlour/get-selected-service/" + parlour_service_id, {
        ...
        $http
        .get(window.location.origin+"/service/api/parlour/get-selected-subservice/" + parlour_service_id, {
            transformRequest: angular.identity,
            headers: {'Content-Type': undefined, 'Process-Data': false}
        })
        .then(function(response){
